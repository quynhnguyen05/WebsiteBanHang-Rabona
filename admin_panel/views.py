...
def cap_nhat_trang_thai_tra(request):
   if request.method == 'POST':
       try:
           # 1. LẤY THÊM SỐ TIỀN CHỐT VÀ GHI CHÚ TỪ GIAO DIỆN ADMIN GỬI LÊN
           if request.content_type and 'multipart/form-data' in request.content_type:
               rr_id = request.POST.get('id')
               new_status = request.POST.get('status')
               refund_amount = request.POST.get('refund_amount')  # Bổ sung
               final_note = request.POST.get('final_note')  # Bổ sung
           else:
               data = json.loads(request.body)
               rr_id = data.get('id')
               new_status = data.get('status')
               refund_amount = data.get('refund_amount')  # Bổ sung
               final_note = data.get('final_note')  # Bổ sung
           ...
           if new_status == 'completed':
               ...
               # ─── 2. ĐÂY LÀ ĐOẠN QUAN TRỌNG NHẤT: GHI ĐÈ SỐ TIỀN VÀ GHI CHÚ MỚI VÀO DB ───
               if refund_amount is not None and str(refund_amount).strip() != '':
                   rr.refund_amount = float(refund_amount)  # Ghi đè số tiền dự tính cũ!
               if final_note is not None and str(final_note).strip() != '':
                   rr.admin_note = str(final_note).strip()  # Ghi chú cho khách biết lý do thay đổi
               # ──────────────────────────────────────────────────────────────────────────
   ...
   # --- LOGIC HIỂN THỊ, TÌM KIẾM VÀ BỘ LỌC CHI TIẾT CỦA BẠN (GIỮ NGUYÊN) ---
   ...
   dang_xu_ly_count = qs.filter(
       status__in=['approved', 'picking', 'returning_to_warehouse', 'received', 'processing']).count()
...
def quan_ly_phi_ship(request):
   ...
   # Lấy toàn bộ danh sách sắp xếp theo tên
   ds_phi_qs = PhiVanChuyen.objects.all().order_by('tenKhuVuc')


   if search_query:
       # XỬ LÝ LỖI SQLITE: Chuyển chuỗi tìm kiếm về chữ thường bằng Python
       search_lower = search_query.lower()


       # Dùng List Comprehension của Python để lọc tiếng Việt chính xác 100%
       ds_phi_list = [p for p in ds_phi_qs if search_lower in p.tenKhuVuc.lower()]


       # Tính phí trung bình thủ công vì lúc này dữ liệu đã thành dạng List
       if ds_phi_list:
       ...
       # Gán lại danh sách để đưa ra giao diện
       ds_phi_qs = ds_phi_list
   else:
       # Nếu không tìm kiếm, dùng hàm aggregate mặc định của DB cho nhanh
